// https://github.com/kevinmehall/nusb/blob/171ee96839547a31e17af5ede9ed044801f06df2/src/maybe_future.rs
use std::{
...
pub mod blocking {
    ...
    impl<R: Send + 'static> BlockingTask<R> {
        ...
        fn spawn(_f: impl FnOnce() -> R + Send + 'static) -> Self {
            panic!(
                "Awaiting blocking syscall without an async runtime: enable the `smol` or `tokio` feature of nusb."
            );
        }
    }
